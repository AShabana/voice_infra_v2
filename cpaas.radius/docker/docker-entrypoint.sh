...
envsubst '
    ...
    ' < /templates/proxy.conf.template > /etc/freeradius/proxy.conf
####################################################################
...
envsubst '
         ...
         ${RADIUS_DB_PASSWORD}
         ${RADIUS_DB_NAME}
         ' < /templates/sql.conf.template > /etc/freeradius/sql.conf
...
envsubst '
         $RADIUS_SQL
         ' < /templates/default.template > /etc/freeradius/sites-available/default
envsubst '
         $RADIUS_SQL
         ' < /templates/inner-tunnel.template > /etc/freeradius/sites-available/inner-tunnel
